...
create or replace procedure matricular (arg_alumno matriculas.alumno%type,
  ...
  Matricula_sin_plazas  exception;
  PRAGMA EXCEPTION_INIT(FK_VIOLATED, -2291);
  PRAGMA EXCEPTION_INIT(Matricula_sin_plazas, -20001);
  

begin 
    if select plazasLibres from grupos where idGrupo=arg_grupo and asignatura=arg_asignatura != 0 then
      INSERT INTO matriculas VALUES(arg_alumno,arg_asig,arg_grupo);
      update grupos set plazasLibres=plazasLibres-1
      where asignatura = arg_asig AND idGrupo=arg_grupo;
