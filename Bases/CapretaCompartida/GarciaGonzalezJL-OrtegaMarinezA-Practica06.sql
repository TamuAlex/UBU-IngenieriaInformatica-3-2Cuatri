...
/**
*Metodo que crea una reserva de un coche 
*Authors: Alejandro Ortega Martinez && Juan Luis Garcia Gonzalez
*Arguemntos de entrada: El nif del cliente a reservar , la matricula del coche y las fechas.
*Devuelve : Nada
*
**/
...
create or replace procedure alquilar(arg_NIF_cliente varchar,
  arg_matricula varchar, arg_fecha_ini date, arg_fecha_fin date) is
    ...
    --Definimos las variables segun enunciado
    ...
    type infoCoche is record( --Guardaremos la informacion relevante al coche a reservar
    ...
    cursor buscarDatosCoche is --Cursor que busca los datos del coche
    ...
    for update ; --De esta forma evitamos que se modifique la informacion mientras se hace la reserva
    
    cursor comprobacionFechas is --Comprobamos que no existen reservas en esas fechas.
    select reservas.idReserva
    from reservas 
    where reservas.matricula=arg_matricula and (
                                                (reservas.fecha_fin>=arg_fecha_ini and reservas.fecha_fin<=arg_fecha_fin) or    --En caos de que la nueva reserva englobe a una existente
                                                (reservas.fecha_ini<=arg_fecha_ini and reservas.fecha_fin<=arg_fecha_fin) or    --En caos de que el final de la reserva existente sea englobado por la nueva reserva
                                                (reservas.fecha_ini<=arg_fecha_fin and reservas.fecha_fin>=arg_fecha_fin) or    --En caso de que el inicio de la reserva existente sea englobado por la nueva reserva
                                                (reservas.fecha_ini<=arg_fecha_ini and reservas.fecha_fin>=arg_fecha_fin)       --En caos de que la reserva existente englobe a la nueva
                                                );
...
begin
  --Paso 1
  if arg_fecha_ini >arg_fecha_fin then --Si la fecha inicio es mayor que la final 
    raise_application_error(-20003, 'El numero de dias sera mayor que cero.');
    end if;
--Paso 2

    open buscarDatosCoche;
    fetch buscarDatosCoche into datos_coche; --Guardamos los datos del coche que vamos a reservar
    
    if buscarDatosCoche%rowcount=0 then --Si no encuntra el coche (guardara 0 lineas)
...
--Paso 3
    ...
    fetch comprobacionFechas into fallos; --Observamos si guarda algun id de una reserva 
    
    if comprobacionFechas%FOUND then --Si hay algun id que guardar 
        close comprobacionFechas;
...
--Paso 4 
    ...
    --Pregunta 1.1 En el caso de que las transacciones fueran serializables, la segunda transaccion en ejecutarse daria el error de que el vehiculo no esta disponible (debido
                    -- al bloqueo de lectura sobre los indices bloquados por el insert. 
    ...
    --Pregunta 2. De la misma forma que en el caso anterior depende del nivel de aislamiento de las trnasaciones. En caso de serializable no seria posible por el bloqueo de indices.
                --Read commited si que es posible. (Pudiendo darse el caso de actualizaciones perdidas)
--Paso 5
    ...
    n_dias:=arg_fecha_fin-arg_fecha_ini;
     ...
     importe_coche:=n_dias*datos_coche.precio_diario; --Calculamos el importe del coche
     
     importe_gasolina:=datos_coche.precio_litro*datos_coche.capacidad_deposito; --Calculamos el importe total de la gasolina
     ...
     /* Pregunta 3: No podrian habernos modificado los datos del cliente debido al insert del caso anterior. Al ser el campo "cliente" de reservas una clave foranea de la tabla cliente, 
     al insertar esa fila en reservas se produce un bloqueo sobre la fila "padre" de esa clave. De esta forma , si alguna fila quiere modificar la informacion se pondra a la espera
     a que termine esta transacion para liberar el bloqueo.
     */
   
    commit;  
    
    exception
    when FK_VIOLATED then --Encapsulamos la excepcion
      rollback;
      raise_application_error(-20001, 'Cliente Inexistente');
      
    when others then --En el caso de que fuera cualquiera otra la lanzamos.
        rollback;
        raise;
